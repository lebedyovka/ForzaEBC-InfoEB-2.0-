...
def analyze_processes(memdump):
    ...
    psscan = run_volatility("psscan", memdump)
    ...
    for pid in hidden:
        proc_name = next((p.get("ImageFileName") or p.get("Name") for p in psscan if p["PID"] == pid), "Unknown")
    ...
    for proc in pslist:
        ...
        user = proc.get("User") or proc.get("Owner") or ""
        session = proc.get("SessionId", -1)

        if not name:
            continue

        if ppid is None and pid != 4:
            print(f"Процесс без PPID: {name} (PID {pid})")

        for sys_name in SYSTEM_PROCESSES:
            if name.lower().startswith(sys_name.replace(".exe", "")) and name != sys_name:
                print(f"Процесс {name} (PID {pid}) может маскироваться под {sys_name}")

        if name in SYSTEM_PROCESSES and name not in ["svchost.exe", "csrss.exe"]:
            if name in seen_system:
                print(f"Найден дубликат системного процесса {name} (PID {pid} и {seen_system[name]})")
            else:
                seen_system[name] = pid

        if session != 0 and user.lower() == "system" and name not in SYSTEM_PROCESSES:
            print(f"Пользовательский процесс {name} работает с SYSTEM правами (PID {pid})")

        if name in SYSTEM_PROCESSES:
            expected_path = SYSTEM_PROCESSES[name].get("path")
            if expected_path and path and expected_path.lower() not in path.lower():
