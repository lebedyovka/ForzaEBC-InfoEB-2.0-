...
def analyze_network(dump):
    ...
    netscan_output = run_cmd(dump, "windows.netscan")

    conns = parse_netscan(netscan_output)
    ...
    for c in conns:
        ...
        if state == "ESTABLISHED":
            ...
            if proc in sus:
                ...
                print()
                print(Fore.RED + "[!] Обнаружено долгоживущее соединение не от браузера")
                print(Fore.WHITE + f"    Процесс: {c['process']} (PID {c['pid']})")
                print(Fore.WHITE + "    Тип: возможный reverse shell")
        ...
        if state == "LISTENING":
            ...
            if port == "53":
                ...
                if proc not in {"svchost.exe", "dns.exe"}:
